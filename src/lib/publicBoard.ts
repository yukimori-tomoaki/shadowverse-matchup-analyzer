...
const toRecord = (row: PublicMatchRow): MatchRecord => ({ id: row.id, date: row.date, displayDate: row.display_date, myDeck: row.my_deck, opponentDeck: row.opponent_deck, turn: row.turn, result: row.result, memo: row.memo });
