...
import { filterMatches, filterPeriod, getDecks, getMyDecks, getOpponentDecks } from "@/lib/matchupCalculator";
...
export default function Home() {
  ...
  const inPeriodRecords = useMemo(() => filterPeriod(records, filters.period, filters.startDate, filters.endDate), [records, filters.period, filters.startDate, filters.endDate]);
  ...
  const periodDecks = useMemo(() => getDecks(inPeriodRecords), [inPeriodRecords]);
  const matrixRowDecks = useMemo(() => (filters.myDeck ? [filters.myDeck] : periodDecks), [filters.myDeck, periodDecks]);
  const existingDecks = useMemo(() => getDecks(filteredRecords), [filteredRecords]);
  const allExistingDecks = useMemo(() => getDecks(records), [records]);
  ...
  const addRecords = async (incoming: MatchRecord[]) => {
    ...
    try {
      ...
      setMessage(detail ? `データを共有サーバーへ保存できませんでした。（${detail}）` : "データを共有サーバーへ保存できませんでした。");
    } finally {
      setSyncing(false);
    }
  };
